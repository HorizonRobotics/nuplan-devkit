...
class NNScene(Scene):
    ...
    def __init__(self, flat_color=True):
        ...
        vertex_perturb3d = """
            ...
            void main()
            {  
                vec4 P = m_view * m_model * vec4(a_position + a_perturb, 1.0);
                v_position = P.xyz / P.w;
                v_normal = normalize(vec3(m_normal * vec4(a_normal, 0.0)));

                gl_Position = m_proj * m_view * m_model * vec4(a_position + a_perturb, 1.0);
        ...
        """
        self.program = gloo.Program(vertex_perturb3d, fragment, version='140')
    ...
    def set_point_perturb_3d(self, arr):
        if self.point_perturb_buffer is None:
            self.point_perturb_buffer = np.zeros(arr.shape[0], [("a_perturb", np.float32, 3)]).view(gloo.VertexBuffer)
        self.point_perturb_buffer['a_perturb'] = arr.astype(np.float32)
        self.program.bind(self.point_perturb_buffer)
    ...
    def set_camera_view(self, m):
        if m.dtype == np.float64:
            m_inv_T = inv(m).T
            m_inv_T = m_inv_T.astype(np.float32)
            self.program['m_view'] = m_inv_T
            self.program['cam_pos_world'] = m[:-1, -1].astype(np.float32)
