...
def load_scene_data(path):
    ...
    if 'intrinsic_matrix' in config:
        ...
        if apath[-3:] == 'xml':
            ...
            with open(apath, 'rb') as f:
                camera_recs = pickle.load(f)
            intrinsic_matrix = [cam_i['intrinsics'] for cam_i in camera_recs]
        else:
    ...
    if 'view_matrix' in config:
        ...
        if apath[-3:] == 'xml':
            ...
            with open(apath, 'rb') as f:
                camera_recs = pickle.load(f)
            # no need to check validity, all extrinsics are from manual labeling
            # view_matrix = [cam_i['extrinsics'] for cam_i in camera_recs]
            # view_matrix = [np.linalg.inv(vm) for vm in view_matrix]
            view_matrix = []
            for i in range(len(camera_recs)):
                vm = camera_recs[i]['extrinsics']
                vm = np.linalg.inv(vm)
                vm[:, 1:3] *= -1
                view_matrix.append(vm)
            camera_labels = [str(ci) for ci in range(len(view_matrix))]
        else:
            view_matrix, camera_labels = extrinsics_from_view_matrix(apath)
    else:
        view_matrix = None
    
    # assert len(view_matrix) == len(intrinsic_matrix)
    print(camera_labels)

    if 'all_img_rec' in config:
        apath = fix_relative_path(config['all_img_rec'], path)
        with open(apath, 'rb') as f:
            all_img_rec = pickle.load(f)
    else:
        all_img_rec = None    
    
    if 'moving_instances' in config:
        apath = fix_relative_path(config['moving_instances'], path)
        with open(apath, 'rb') as f:
            moving_instances = pickle.load(f)
    else:
        moving_instances = None    

    if 'origin_pose' in config:
        apath = fix_relative_path(config['origin_pose'], path)
        with open(apath, 'rb') as f:
            origin_pose = pickle.load(f)
    else:
        origin_pose = None    
    ...
    'all_img_rec': all_img_rec,
    'moving_instances': moving_instances,
...
def load_scene_data_minimal(base_dir):
    pointcloud_path = '{}/merge_points.ply'.format(base_dir)
    camera_recs_path = '{}/images.pkl'.format(base_dir)
    origin_pose_path = '{}/origin_pose.pkl'.format(base_dir)
    moving_instances_path = '{}/moving_instances.pkl'.format(base_dir)

    print('loading pointcloud...')
    pointcloud = import_model3d(pointcloud_path)
    with open(camera_recs_path, 'rb') as f:
        camera_recs = pickle.load(f)
        intrinsic_matrix = [cam_i['intrinsics'] for cam_i in camera_recs]
        distortion = [cam_i['distortion'] for cam_i in camera_recs]
        view_matrix = []
        for i in range(len(camera_recs)):
            vm = camera_recs[i]['extrinsics']
            vm = np.linalg.inv(vm)
    ...
    with open(origin_pose_path, 'rb') as f:
        origin_pose = pickle.load(f)
    with open(moving_instances_path, 'rb') as f:
        moving_instances = pickle.load(f)
