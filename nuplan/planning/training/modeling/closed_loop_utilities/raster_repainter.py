...
def repaint_raster(
    ...
    rot_deg = (
        -principal_value(
            torch.atan2(bounded_pose_mats[:, 1, 0], bounded_pose_mats[:, 0, 0])
        )
        * 180.0
        / math.pi
    ).type_as(raster)
    ...
    affine_mat[:, 0, 2] = (
        affine_mat[:, 0, 2] + bounded_pose_mats[:, 1, 2] / target_pixel_size
    )
    affine_mat[:, 1, 2] = (
        affine_mat[:, 1, 2] + bounded_pose_mats[:, 0, 2] / target_pixel_size
    )
