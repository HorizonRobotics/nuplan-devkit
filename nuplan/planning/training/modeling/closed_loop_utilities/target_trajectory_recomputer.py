...
class TargetTrajectoryRecomputer(object):
    ...
    def __init__(
        ...
        self._smoother = ConstrainedNonlinearSmoother(
            trajectory_len=trajectory_len, dt=time_interval
        )
    ...
    def reset(self):
        self._smoother = ConstrainedNonlinearSmoother(
            trajectory_len=self._trajectory_len, dt=self._time_interval
        )
    ...
    def recompute(
        ...
        ref_traj = [[0.0, 0.0, 0.0]] + original_target.tolist()
        ...
        self._smoother.set_reference_trajectory(
            x_curr=x_curr, reference_trajectory=ref_traj
        )
        try:
        ...
        if not sol.stats()["success"]:
            ...
            ego_perturb = (
                np.linalg.inv(pose_matrix.cpu().numpy()) @ ego_perturb
            )
